  ...
  <div class="min-h-screen">
    ...
    <div class="bg-white border-b">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav class="flex gap-4 overflow-x-auto whitespace-nowrap scrollbar-hide">
          <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 border-b-2 border-blue-600 py-4 px-3 text-sm font-medium text-blue-600 duration-200">
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="space-y-8">
        ...
        <!-- Stats Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 {{ Auth::user()->role === 'super_admin' ? 'lg:grid-cols-4' : 'lg:grid-cols-2' }} gap-6">
          <!-- Total Members Card -->
@if(Auth::user()->role === 'super_admin')
...
@endif
          ...
          <!-- Attendance Created Card -->
          <div class="bg-white rounded-lg shadow border">
            <div class="px-6 py-4">
              <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-2">
                <h3 class="text-sm font-medium text-gray-600">Attendance Created</h3>
                <svg class="h-5 w-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path>
                </svg>
              </div>
              <div class="text-3xl font-semibold text-gray-900 text-right">{{ $totalAttendanceSessions }}</div>
              <a href="{{ route('attendance') }}" class="text-sm text-blue-600 hover:underline mt-2 inline-block">Manage attendance</a>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow border">
            <div class="px-6 py-4">
              <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-2">
                <h3 class="text-sm font-medium text-gray-600">Avg. Attendance / Session</h3>
        ...
        <div class="bg-white rounded-lg shadow border">
          <div class="px-6 py-4 border-b">
            <h3 class="text-lg font-semibold">Recent Attendance Overview</h3>
          </div>
          <div class="px-6 py-4">
          ...
          <div class="px-6 py-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <a href="{{ route('members.index') }}" class="flex items-center gap-3 p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div>
                  <div class="font-medium text-gray-900">{{ Auth::user()->role === 'super_admin' ? 'Add New Member' : 'View Members' }}</div>
                  <div class="text-sm text-gray-600">{{ Auth::user()->role === 'super_admin' ? 'Register a new church member' : 'Search members and open their QR codes' }}</div>
                </div>
              </a>
...
<script>
  const recentEventLabels = @json($recentAttendanceSessions->map(fn ($session) => $session->attendance_name . ' - ' . ($session->event->event_name ?? 'No event'))->values());
  const recentEventData = @json($recentAttendanceSessions->pluck('attendance_count'));
